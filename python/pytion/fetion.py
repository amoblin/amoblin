...
class GUI():
    def __init__(self):
        ...
        self.frame=[]
        self.frame.append(Tkinter.Frame())
        self.frame.append(Tkinter.Frame())
        self.scrollbar=Tkinter.Scrollbar(self.frame[0])
        self.scrollbar.pack(side=Tkinter.RIGHT,fill=Tkinter.Y)
        self.MessageOut=Tkinter.Listbox(self.frame[0],height=25)
        self.MessageOut['yscrollcommand']=self.scrollbar.set
        self.MessageOut.pack(expand=1,fill=Tkinter.BOTH)
        self.scrollbar['command']=self.MessageOut.yview
        self.frame[0].pack(expand=1,fill=Tkinter.BOTH)

        self.MessageIn=Tkinter.Entry(self.frame[1],width=60)
        self.MessageIn.pack(expand=1,fill=Tkinter.X,pady=10,padx=15)
        self.MessageIn.bind("<Return>",self.SendMessageTo)
        self.MessageIn.focus_set()
        self.frame[1].pack()
        self.root.mainloop()

    def SendMessageTo(self,event):
        self.send_data=""
        self.send_data=self.MessageIn.get()
        if self.send_data:
            self.MessageOut.insert(Tkinter.END,self.send_data)
