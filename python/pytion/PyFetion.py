...
FetionConfigXML = """<config><user mobile-no="%s" /><client type="PC" version="4.0.3390" platform="W6.1" /><servers version="0" /><service-no version="0" /><parameters version="0" /><hints version="0" /><http-applications version="0" /><client-config version="0" /><services version="0" /></config>"""

FetionLoginXML2 = """<args><device accept-language="default" machine-code="CE874C7A4ABE09F62CACC3B01BEA5527"/><caps value="1FFF" /><events value="7F" /><user-info mobile-no="%s" user-id="" /> <personal version="0" attributes="v4default;alv2-version;alv2-warn;dynamic-version" /><custom-config version="334490711" /><contact-list version="334446374" buddy-attributes="v4default" /></user-info><credentials domains="fetion.com.cn;m161.com.cn;www.ikuwa.cn;games.fetion.com.cn;turn.fetion.com.cn;pos.fetion.com.cn" /> <presence><basic value="%s" desc="" /><extendeds /></presence></args>"""
...
class SIPC():
    global FetionVer
    global FetionSIPP
    global FetionLoginXML,FetionLoginXML2
    ...
    def get(self,cmd,arg,*extra):
        ...
        if cmd == "REG":
            ...
            if arg == 1:
                ...
                self._content += "CN: " + cnonce + "\r\n"
                self._content += 'CL: type="pc",version="4.0.2510"' + "\r\n"
                pass
            if arg == 2:
                body = FetionLoginXML2 % (self.mobile_no,self.presence)
                nonce = re.search('nonce="(.{32})"',extra[0]).group(1)
                key = re.search('key="(.{262})"',extra[0]).group(1)
                signature = re.search('signature="(.*)"',extra[0]).group(1)
                response=self.__get_response_rsa(nonce,key[:256],key[256:])
        ...
        #if len(body) != 0: #DO NOT need L anymore after v4.
    ...
    def __get_response_rsa(self,nonce,hex_modulus,hex_public):
        aeskey = md5(str(uuid1())).hexdigest().upper()
        aes_hex = binascii.a2b_hex(aeskey)

        #print len(aeskey),len(aes_hex)

        password = self.hash_password_v2()
        raw_str = nonce + password + aes_hex          #nonce:32 + 40 + 32 = 104
    ...
    while retry:
        try:
            ...
            if code == 404 or code == 500:
                #error from here since SHIA v4 Protocol.
...
class PyFetion(SIPC):
    ...
    def __init__(self,mobile_no,passwd,login_type="TCP",debug=False):
        ...
        self.__sipc_url   = "https://uid.fetion.com.cn/ssiportal/SSIAppSignInV4.aspx"   #fetion 2010 v4.0
    ...
    def __get_uri(self):
        url = self.__sipc_url+"?mobileno="+self.mobile_no
        url += "&domains=fetion.com.cn%3bm161.com.cn%3bwww.ikuwa.cn"
        url += "&v4digest-type=1"
        url += "&v4digest=" + sha1("fetion.com.cn:" + self.passwd).hexdigest()
        d_print(('url',),locals())
    ...
    def hash_password_v2(self):
        res = sha1("fetion.com.cn:" + self.passwd).hexdigest().upper()
        res = sha1(binascii.a2b_hex(self.userid + res )).hexdigest().upper()
